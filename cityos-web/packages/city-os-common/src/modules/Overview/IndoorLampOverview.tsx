...
const OverviewContent: VoidFunctionComponent<OverviewContentProps> = ({
  ...
  const overviewContent = useMemo(() => {
    ...
    contentWithSensors.splice(1, 0, {
      ...
      title: t('switch'),
      type: 'switch',
      value: sensorValues?.[deviceId]?.switch?.value !== 0,
    });
...
interface IndoorLampOverviewProps extends ComponentProps<typeof Overview> {
  name: string;
...
const IndoorLampOverview: VoidFunctionComponent<IndoorLampOverviewProps> = ({
  name,
  device,
  shownSensors = [],
...
export default IndoorLampOverview;
