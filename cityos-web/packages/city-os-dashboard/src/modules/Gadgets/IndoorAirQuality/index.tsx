...
import IndoorAirQualityConfig from './IndoorAirQualityConfig';
...
const useStyles = makeStyles((theme) => ({
  ...
  revenue: {
    color: theme.palette.gadget.indoorAirQuality,
  },
...
interface IndoorAirQualityProps {
...
const IndoorAirQuality: VoidFunctionComponent<IndoorAirQualityProps> = ({
  config,
  enableDuplicate,
  isDraggable,
  onDelete,
  ...
  >(SENSOR_VALUES_AVG_HISTORY, {
    variables: {
      deviceId,
      sensorId: SensorId.BANPU_INDOOR_METER_CO2,
      start: new Date(endTime - timeInRange),
      end: new Date(endTime - 1), // query to end of the period
      interval,
    },
    onCompleted: (data) => {
      setCurve(getCurve(id, data.sensorValuesAvgHistory, 'indoorAirQuality'));
      console.info('current indoorAirQuality', data);
  ...
  >(SENSOR_VALUES_AVG_HISTORY, {
    ...
    onCompleted: (data) => {
      ...
      console.info('past indoorAirQuality', data);
    },
    onError: () => {
      setPastCurve({
        key: `past-${id}`,
  ...
  >(getSensorValueChanged(SensorType.GAUGE), {
    variables: { deviceId, sensorId: SensorId.BANPU_INDOOR_METER_CO2 },
  });
  ...
  // const dataValueList = useMemo(
  //   () => lodash.compact(currentData?.sensorValuesAvgHistory.map((point) => point.value)) || [],
  //   [currentData?.sensorValuesAvgHistory],
  // );

  // const minDataValue = useMemo(() => lodash.min(dataValueList), [dataValueList]);
  // const manDataValue = useMemo(() => lodash.max(dataValueList), [dataValueList]);
  // const avgDataValue = useMemo(() => Math.round(lodash.mean(dataValueList)), [dataValueList]);
  ...
  const dangerousLevels = useMemo(() => {
    const value = subscribeData?.sensorValueChanged.data.value;

    if (value === undefined) return '---';
    if (value >= 1600) return 'Bad';
    if (value >= 1100) return 'Mediocre';
    if (value >= 900) return 'Fair';
    if (value >= 700) return 'Good';
    return 'Exellent';
  }, [subscribeData?.sensorValueChanged.data.value]);
  ...
  return (
    <GadgetBase
      ...
      ConfigComponent={IndoorAirQualityConfig}
    >
        ...
        >
          ...
          >
            <Grid item xs={2} container justify="center" alignItems="center" wrap="nowrap">
              <Avatar className={classes.avatar}>
                <DeviceIcon type={DeviceType.BANPU_INDOOR_METER} width={24} height={24} />
              </Avatar>
            </Grid>
            <Grid item xs={5} container direction="column" className={classes.textWrapper}>
              <Typography variant="subtitle1" noWrap className={classes.text}>
                {`${t('dashboard:Air Quality')} (${t('dashboard:Now')})`}
              </Typography>
            ...
            <Grid item xs={5} container>
              ...
              <Grid item xs={12}>
                <Typography align="right" className={classes.subtitle}>
                  {`${t('dashboard:CO_2')} (${t('dashboard:ppm')}) / `}
                  <span style={{ fontWeight: 600 }}>{dangerousLevels}</span>
                </Typography>
              </Grid>
            </Grid>
            <Grid item xs={12}>
          ...
          >
            ...
            <LineCharts
              ...
              pastCurve={pastCurve}
              duration={duration}
              valueParser={(value) => `${value.toLocaleString('en-US')} ppm`}
              labelType="default"
            />
          </Grid>
        </Grid>
...
export default memo(IndoorAirQuality);
