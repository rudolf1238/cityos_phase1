...
interface DeviceLayoutProps<T extends GadgetConfigType<ConfigFormType.DEVICE_LAYOUT>> {
  gadgetType: T;
  saveType: ConfigSaveType;
  onUpdateGadget: (newGadgetConfig: GadgetConfig<ConfigFormType.DEVICE_LAYOUT>) => void;
  config?: GadgetConfig<ConfigFormType.DEVICE_LAYOUT>;
}

const DeviceLayout = <Type extends GadgetConfigType<ConfigFormType.DEVICE_LAYOUT>>({
  gadgetType,
  saveType,
  onUpdateGadget,
  config: initConfig,
}: DeviceLayoutProps<Type>): ReturnType<VoidFunctionComponent<DeviceLayoutProps<Type>>> => {
  ...
  } = useForm<GadgetConfig<ConfigFormType.DEVICE_LAYOUT>>({
    mode: 'onChange',
    defaultValues: {
      id: initConfig?.id || '',
  ...
  const handleSave = useCallback<SubmitHandler<GadgetConfig<ConfigFormType.DEVICE_LAYOUT>>>(
    async (config) => {
...
export default memo(DeviceLayout);
